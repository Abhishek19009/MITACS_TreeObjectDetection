...
class PillarLayer(nn.Module):
    ...
    def forward(self, batched_pts):
        ...
        return:
            ...
            num_points_per_pillar: (p1 + p2 + ... + pb, ), (b: batch_size)
        '''
        
        pillars, coors, npoints_per_pillar = [], [], []
        for i, pts in enumerate(batched_pts):
            
            # THIS IS THE MAIN PROBLEM, WE ARE GETTING 0 VOXELS
            voxels_out, coors_out, num_points_per_voxel_out = self.voxel_layer(pts)
            
            
#             print(voxels_out.shape, coors_out.shape, num_points_per_voxel_out.shape)
...
class Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
#         print("Backbone inputs: ", x)
        print("Backbone inputs: ", x.shape)
        outs = []
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        x: (bs, 384, 248, 216)
        return: 
              bbox_cls_pred: (bs, n_anchors*2, 248, 216) 
...
class PointPillars(nn.Module):
    def __init__(self, 
                ...
                point_cloud_range=[-5, -5, 0, 23, 23, 25],
#                 point_cloud_range=[0, -39.68, -1, 69.12, 39.68, 5],
                max_num_points=32,
        ...
        ranges = [[-5, -5, 0, 23, 23, 25], [-5, -5, 0, 23, 23, 25]]
        # ranges = [[-5, -5, 0, 23, 23, 25]]

#         ranges = [[0.34, 0.26, 3.71, 5.5, 5.5, 18.8], [0.63, 0.50,
        #         5.61, 4.0, 5.1, 17.0]]
#         ranges = [[0, -39.68, -0.6, 69.12, 39.68, -0.6], [0, -39.68, -0.6, 69.12, 39.68, -0.6]]

        sizes = [[2.5, 2.5, 20], [2.5, 2.5, 20]]
        # sizes = [[2.5, 2.5, 20]]
#         sizes = [[4, 4, 20], [4, 4, 20]]
        rotations = [-1.5, 4.5]
        ...
        # CAUTION: this value changed from 0.05 to 0.1
        self.nms_thr = 0.05

        # CAUTION: this value changed from 0.1 to 0.01
        self.score_thr = 0.3
    ...
    def get_predicted_bboxes_single(self, bbox_cls_pred, bbox_pred, bbox_dir_cls_pred, anchors):
        '''
        bbox_cls_pred: (n_anchors*2, 248, 216)
...
#         print(bbox_cls_pred.shape, bbox_pred2d.shape, bbox_pred.shape, bbox_dir_cls_pred.shape)
...
#         print("Nan or not: ", torch.isnan(batched_pts).any())
        
        # batched_pts: list[tensor] -> pillars: (p1 + p2 + ... + pb, num_points, c), 
        #                              coors_batch: (p1 + p2 + ... + pb, 1 + 3), 
        #                              num_points_per_pillar: (p1 + p2 + ... + pb, ), (b: batch size)
        pillars, coors_batch, npoints_per_pillar = self.pillar_layer(batched_pts)
        # print(pillars.shape, coors_batch.shape)
        ...
        # print("Pointpillar outputs: ", bbox_cls_pred.shape, bbox_pred.shape, bbox_dir_cls_pred.shape)
